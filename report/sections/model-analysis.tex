...
The observations in \autoref{sec:results} support this performance. In all mentioned metrics, apart from \MAPE, \xgbr scores significantly better than other models tested. Especially when looking at \RT where \xgbr scores under 0.5\%. 
...
As shown on both \autoref{fig:xgbr-one} and \autoref{fig:xgbr-five}, while the precision of the model has decreased, \xgbr is still capable of detecting the sudden change in the temperature difference, which again, shows the capabilities of \xgbr.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{report/img/xgb-1m-78.png}
    \caption{\xgbr predicting 1 minute}
    \label{fig:xgbr-one}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{report/img/xgb-5m-78.png}
    \caption{\xgbr predicting 5 minute}
    \label{fig:xgbr-five}
\end{figure}
