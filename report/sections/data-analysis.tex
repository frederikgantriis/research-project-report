...
All data is collected from the preheater tower mentioned in \autoref{sec:introduction}. This includes the following datapoints:

\begin{itemize}
    \item \texttt{PV1\_A\_Feed\_to\_Oven\_tons\_per\_day}: How many tons of slurry are fed to the kiln per day.
...
All of these values were then implemented twice: Once for the A-side and once for the B-side of the pre-heater tower. Numbers on the B-side match the numbers on the A-side, with 10 added to each of them, meaning PV4 on the A-side is named PV14 on the B-side.

In addition, two features were computed from the previously mentioned features:

\begin{itemize}
    \item \texttt{pressure\_diff}: The difference between top (PV2) and bottom (PV4)
    \item \texttt{temperature\_diff}: The difference between top (PV3) and bottom (PV5)
\end{itemize}

These two features were added, based on expert knowledge from \ap. Both features are used by the operators as critical signs that a cyclone blockage might be appearing. Therefore, predictions are mainly made on the difference in temperature. This means that the target variable to predict is the computed \texttt{temperature\_diff}.
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{report/img/descriptive_stats.png}
...
Both figures gives us a clear picture. Firstly, there is a strong general correlation. Secondly, sensors from one side might have a strong correlation with either the same respective sensor or others placed on the opposite side. Furthermore, there is a general pattern showing that sensors sitting consequently one another has a high correlation. This information determines the experimentation of the models, since variant with data from both sides and from a single side would have to be tested. 

\subsubsection{Autocorrelation analysis}
\label{sec:autocorrelation}

In addition to the provided correlation analysis, an autocorrelation analysis was performed to analyse correlation over time. Autocorrelation measures the linear relationship between a time series and a lagged version of itself. It quantifies how strongly past values predict current values at different time lags, indicating the presence of temporal dependence or repeating patterns. High autocorrelation at a given lag suggests that observations separated by that lag are systematically related rather than independent.

In this analysis, it is clear that patterns emerge over time for both PV3 (\autoref{fig:PV3-autocorrelation}) and PV5 (\autoref{fig:PV5-autocorrelation}); the two sensors used to create \texttt{temperature\_diff}. Even when the correlation switches from being a high positive correlation to being a negative correlation, there is still a clear pattern shown that is present in both of the mentioned figures. This means, that the data should be kept in order when splitting the dataset, to ensure that patterns collected over time aren't lost. 

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{report/img/PV3_A_Bottom_Temperature_autocorrelation.png}
    \caption{Autocorrelation for PV3}
    \label{fig:PV3-autocorrelation}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{report/img/PV5_A_First_Step_Bottom_Temperature_autocorrelation.png}
    \caption{Autocorrelation for PV5}
    \label{fig:PV5-autocorrelation}
\end{figure}